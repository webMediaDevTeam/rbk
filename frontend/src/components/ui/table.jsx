...
function TableRow({ className, ...props }) {
  return (
    <tr
      data-slot="table-row"
      className={cn("border-b border-border transition-colors hover:bg-primary/10 data-[state=selected]:bg-primary/10 data-[state=selected]:text-primary", className)}
      {...props}
    />
  );
}
...
function TableHead({ className, ...props }) {
  return (
    <th
      ...
      className={cn("h-10 px-2 text-start align-middle font-medium whitespace-nowrap text-foreground *:[[role=checkbox]]:translate-y-0.5", className)}
      {...props}
    />
  );
}
